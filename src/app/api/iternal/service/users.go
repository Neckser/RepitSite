...
func (s *UserService) GetAllUsers() ([]models.AdminUsers, error) {
	...
	query := `SELECT tutor_id as id, first_name || ' ' || last_name as fullname, 'Репетитор' as role FROM tutors
		UNION ALL
		SELECT student_id as id, first_name || ' ' || last_name as fullname, 'Студент' as role FROM students`
